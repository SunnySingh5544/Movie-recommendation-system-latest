...
/* ── Movie card: poster fills the card, button is an invisible overlay ── */
.movie-card {
    position: relative;
    border-radius: 12px;
    overflow: hidden;
...
/* Make Streamlit buttons that sit inside a card invisible overlays */
...
# ── Helper: render a movie grid ───────────────────────────────────────────────
def render_movie_grid(movies, cols=5, badge_text=None, grid_id="grid"):
    """
    FIX 1 — grid_id makes every button key globally unique, preventing the
             DuplicateWidgetID error that silently killed the recommendations.

    FIX 2 — The card HTML uses `position: relative` and the st.button is
             given the class `card-btn` via a container key trick so the CSS
             stretches it to cover the whole poster — making the entire card
             clickable, not just an invisible button underneath.
    """
    columns = st.columns(cols)
    for i, movie in enumerate(movies):
        with columns[i % cols]:
            ...
            # Wrap card HTML + invisible overlay button together so the button
            # sits on top of the card via CSS (position: absolute; inset: 0).
            with st.container():
                st.markdown(html, unsafe_allow_html=True)
                # KEY FIX: prefix with grid_id so trending + rec grids never clash
                btn_key = f"{grid_id}_btn_{i}_{title[:30]}"
            ...
            if res.status_code != 200:
                ...
                if "error" in data:
                    ...
                    with right:
                        ...
                        st.markdown(f'<div class="detail-title">{movie["title"]}</div>',
                                    unsafe_allow_html=True)

                        meta_parts = [p for p in [release, f"{runtime} min" if runtime else None] if p]
                        st.markdown(f'<div class="detail-meta">{" · ".join(meta_parts)}</div>',
                                    unsafe_allow_html=True)
                        ...
                        overview = movie.get("overview", "")
                        if overview:
                            st.markdown(f'<div class="detail-overview">{overview}</div>',
                                        unsafe_allow_html=True)
                    ...
                    if recs:
                        ...
                        # FIX: use grid_id="recs" so these keys never clash with trending
                        render_movie_grid(normalised, cols=5, grid_id="recs")
...
if not query:
    ...
    try:
        ...
        if res.status_code == 200:
            trending = res.json()[:10]
            # FIX: use grid_id="trending" so these keys never clash with recs
            render_movie_grid(trending, cols=5, badge_text="TRENDING", grid_id="trending")
        else:
            st.error("Failed to load trending movies ❌")
